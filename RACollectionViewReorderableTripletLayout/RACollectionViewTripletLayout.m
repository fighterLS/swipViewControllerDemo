...
- (CGSize)collectionViewContentSize
{
    ...
    for (NSInteger i = 0; i < self.collectionView.numberOfSections; i++) {
        ...
        NSInteger numberOfLines = ceil((CGFloat)[self.collectionView numberOfItemsInSection:i] / 3.f);
        CGFloat lineHeight = numberOfLines * ([_smallCellSizeArray[i] CGSizeValue].height + _lineSpacing) - _lineSpacing;
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cellSize
    CGFloat smallCellSideLength = (1.f * (_collectionViewSize.width - _insets.left - _insets.right) -2*_itemSpacing) / 3.f;// (largeCellSideLength - _itemSpacing) / 2.f;
    ...
    for (NSInteger i = 0; i <= indexPath.section - 1; i++) {
        ...
        sectionHeight += lines * (_lineSpacing + smallCellHeight) + _sectionSpacing;
    ...
    CGFloat lineOriginY = _smallCellSize.height * line + sectionHeight + lineSpaceForIndexPath + _insets.top;
    ...
    if (indexPath.item%3==0) {
        ...
        {
            attribute.frame =CGRectMake(_insets.left+_itemSpacing+_smallCellSize.width, lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }else if(indexPath.item%3==2)
        {
             attribute.frame =CGRectMake(_insets.left+2*(_itemSpacing+_smallCellSize.width), lineOriginY, _smallCellSize.width, _smallCellSize.height);
        }
